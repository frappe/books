...
/**
 * Telemetry
 * Disabled automatically in Electron packaged builds
 * because navigator.sendBeacon only works over HTTP(S)
 */
...
// Disable telemetry in packaged Electron apps
const TELEMETRY_ENABLED = isHttpContext && !isElectron;
...
export class TelemetryManager {
  ...
  stop() {
    if (!this.started || !TELEMETRY_ENABLED) {
      return;
    }
  ...
  log(verb: Verb, noun: Noun, more?: Record<string, unknown>) {
    ...
    if (!this.#started && this.fyo.db.isConnected) {
      void this.start().then(() => this.#sendBeacon(verb, noun, more));
      return;
    }
  ...
  #sendBeacon(verb: Verb, noun: Noun, more?: Record<string, unknown>) {
    ...
    // Guarded execution — NEVER crashes Electron
    try {
      navigator.sendBeacon(this.#url, data);
    } catch {
      // swallow silently (never crash renderer)
    }
  }

  async #setCreds() {
    if (!TELEMETRY_ENABLED || this.hasCreds) {
      return;
    }
