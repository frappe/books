<style scoped>
  ...
  .dropdown2 :deep(button), .btn-left :deep(button), .date-start :deep(.rounded), .date-stop :deep(.rounded) {
    border-top-right-radius: 0;
    border-bottom-right-radius: 0;
  ...
  .date-start :deep(input + div), .date-stop :deep(input + div),
  .date-start :deep(input), .date-stop :deep(input) {
    height: 2rem;
    --tw-text-opacity: 1;
...
<template>
  <div style="
    ...
    >
    <div class="dropdown1">
      <Dropdown ref="dropdownPeriod" class="text-sm" :items="periodOptions" right>
        <template
          #default="{
            toggleDropdown,
            highlightItemUp,
    ...
    <div class="dropdown2"
      ...
      >
      <Dropdown ref="dropdownAnchor" class="text-sm" :items="anchorOptions" right>
        <template
          #default="{
            toggleDropdown,
            highlightItemUp,
    ...
    <div class="btn-left">
      <Button icon @click="prev"><feather-icon name="chevron-left" class="ms-1 w-3 h-3" /></Button>
    </div>
    <div class="date-start">
      <DateComp :df="startDateField" :value="startDate.toISODate()" @change="changeStartDate"></DateComp>
    </div>
    <div class="date-stop">
      <DateComp :df="stopDateField" :value="stopDate.minus({days: 1}).toISODate()" @change="changeStopDate"></DateComp>
    </div>
    <div class="btn-right">
      <Button icon @click="next"><feather-icon name="chevron-right" class="ms-1 w-3 h-3" /></Button>
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  mounted() {
    ...
    this.anchorOptions = Object.keys(this.anchorSelectorMap).map((option) => {
  ...
  methods: {
    async selectAnchor(value: PeriodKey) {
      ...
      if(this.anchorValue == 'to_date') {
        ...
        switch(this.value) {
          case 'Y': this.startDate = this.startDate.startOf('year'); break;
          case 'S': this.startDate = this.startDate.startOf('month').set({
                      month: this.startDate.month<7 ? 1 : 7})
                    break;
          case 'Q': this.startDate = this.startDate.startOf('quarter'); break;
          case 'M': this.startDate = this.startDate.startOf('month'); break;
        }
        this.stopDate = this.startDate.plus(this.period(this.value, 1))
      } else if (this.anchorValue == 'fiscal') {
        const fys = DateTime.fromJSDate(await fyo.getValue(ModelNameEnum.AccountingSettings, 'fiscalYearStart'))
        const fye = DateTime.fromJSDate(await fyo.getValue(ModelNameEnum.AccountingSettings, 'fiscalYearEnd'))
        const duration = fye.plus({days: 1}).diff(fys, ['years', 'months', 'days'])
        ...
        for (let i = 12; i > 0 && this.startDate.plus(this.period(this.value, 1)).toJSDate() < today.toJSDate(); i--) {
          this.startDate = this.startDate.plus(this.period(this.value, 1))
        }

        this.stopDate = this.startDate.plus(this.value == 'Y' ? duration : this.period(this.value, 1))
      }

      this.emitChanged();
      (this.$refs.dropdownAnchor as InstanceType<typeof Dropdown>).toggleDropdown(
        false
      );
    },
    selectOption(value: PeriodKey) {
      ...
      (this.$refs.dropdownPeriod as InstanceType<typeof Dropdown>).toggleDropdown(
        false
      );
    },
    changeStartDate(d) {
    ...
    period(val, n) {
      switch(val) {
        case 'Y': return {years: n}
        case 'S': return {months: n * 6}
        case 'Q': return {months: n * 3}
