...
export default {
  ...
  methods: {
    append: function(value) {
      switch (this.focused) {
        ...
        case 'CASH_DEC':
          ...
          break;
        case 'CHEQUE':
          this.cheque = this.cheque * 10 + value;
          break;
        case 'CHEQUE_DEC':
    ...
    del: function() {
      switch (this.focused) {
        case 'CASH':
          this.cash = (this.cash - this.cash % 10) / 10;
          break;
        case 'CASH_DEC':
          this.cash = parseFloat(this.cash.toString().slice(0, -1));
          this.cashDecCount++;
          if (this.cashDecCount == 0) this.focused = 'CASH';
          break;
          break;
        case 'CHEQUE':
          this.cheque = (this.cheque - this.cheque % 10) / 10;
          break;
        case 'CHEQUE_DEC':
          this.cheque = parseFloat(this.cheque.toString().slice(0, -1));
          this.chequeDecCount++;
          if (this.chequeDecCount == 0) this.focused = 'CHEQUE';
          break;
          break;
      }
    },
    addDP: function(value) {
      switch (this.focused) {
        case 'CASH':
          this.focused = 'CASH_DEC';
          break;
        case 'CHEQUE':
          this.focused = 'CHEQUE_DEC';
          break;
      }
    },
    changeFocus: function(target) {
      ...
      switch (target) {
        case 'CASH':
          if (this.cashDecCount < 0) this.focused = 'CASH_DEC';
          break;
        case 'CHEQUE':
          if (this.chequeDecCount < 0) this.focused = 'CHEQUE_DEC';
          break;
      }
    },
    submit() {
  ...
  computed: {
    outstandingAmount() {
      ...
      return (
        this.grandTotal -
        parseFloat(this.cash) -
        parseFloat(this.cheque)
      ).toFixed(2);
    }
  }
};
