...
export class BespokeQueries {
  ...
  static async migrateExpenseDescription(db: DatabaseCore): Promise<void> {
    ...
    // Check if numberSeries column exists
    const hasNumberSeries = columns.some((col: any) => col.name === 'numberSeries');
    if (!hasNumberSeries) {
      // Add numberSeries column with default value
      await db.knex!.raw('ALTER TABLE Expense ADD COLUMN numberSeries TEXT DEFAULT "EXP-"');
      console.log('✅ Added numberSeries column to Expense table');
    }

    // Check if name column exists (for the expense number display)
    const hasName = columns.some((col: any) => col.name === 'name');
    if (!hasName) {
      // Name will be auto-generated by the naming system
      console.log('ℹ️ Name column will be created by schema system');
    }
  }
